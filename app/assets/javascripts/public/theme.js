jQuery(function($) {
    ...
    $('.carousel').on('slid', function(e) {
        ...
        var index = t.find('.item.active').index(),
            navs = [t.closest('.news-slider').find('.carousel-nav ul'), t.find('.carousel-nav ul')];
        for (var key in navs) {
    ...
    $('.carousel').each(function() {
        var t = $(this),
            nav = [t.find('.carousel-nav ul'), t.closest('.news-slider').find('.carousel-nav ul')];
        for (var key in nav) {
            if (nav[key].length > 0) {
    ...
    if (timmer) {
        headline_interval = setInterval(headline_rotate, timmer);
    ...
    function headline_rotate() {
        ...
        $(".headlines li:first").animate({
            marginTop: ((current_headline - headline_count) * 20) + "px"
        }, "slow");
    }
    var change_category_tab = function(obj, catid) {
        ...
        obj.addClass('active');
        widget.find('.row-fluid.active').hide().removeClass('active').end().find('.cat-' + catid).fadeIn('slow').addClass('active');
    }
